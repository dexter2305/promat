...
public class StarMatchingEngine {
	...
	private void init(String strConfigFileName) throws Exception {
		...
		// ,Star.POORATTATHI_123,Star.POORATTATHI_4,Star.UTHRATTATHI,Star.REVATHI));
		...
		this.maleProfiles = importCandidates(importConfigProps, malesExcelFileName,
				Arrays.asList((malesExcelFileSheetNames.split(","))));
		this.femaleProfiles = importCandidates(importConfigProps, femalesExcelFileName,
				Arrays.asList((femalesExcelFileSheetNames.split(","))));
		logger.info("EXIT - init()");
	}

	private Collection<Candidate> importCandidates(Properties importConfigProps, String excelFileName,
			List<String> excelFileSheetNames) throws Exception {
	...
	private Set<MatchingProfile> matchProfile(Star star) {
		...
		profiles.stream().forEach((matchAgainstCandidate) -> {
			...
			if (matchResult != null) {
				...
				// logger.info("No MATCH profile with Id :"+
				// matchAgainstCandidate.getId());
			}
		});
		...
		logger.info("matchProfile(long userId) - EXIT : Match completed for star " + star + ". # of matches found."
				+ starMatches.size());
		return starMatches;
	...
	private MatchingProfile match(Star mfs, Candidate matchAgainstCandidate) {
		logger.debug("match(Candidate matchForCandidate, Candidate matchAgainstCandidate) - ENTER :"
				+ matchAgainstCandidate.getId());
