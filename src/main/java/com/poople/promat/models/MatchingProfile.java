...
public class MatchingProfile {
	...
	public static ArrayList<String> getListFormat() {
		return new ArrayList<String>(Arrays.asList("Id", "Name", "Star", "MatchingId", "MatchingName", "Kulam", "Star",
				"Strength", "MatchType", "Dob", "Tob", "Age", "BirthPlace", "RahuKethu", "Sevvai", "Height",
				"MaritalStatus", "Qualification", "WorkPlace", "Detail 1", "Detail2", "Info"));
	}
	...
	public List<String> toList() {
		...
		if (profile != null) {
			...
			strMat = new String[] { getStringValue(id), getStringValue(name), convertToString(star),
					getStringValue(profile.getId()), profile.getName(), profile.getKulam(),
					convertToString(matchedStar), getStringValue(strength), convertToString(match), convertToString(d),
					convertToString(t), getStringValue(age), birthPlace, rk, sev,
					(profile.getPhysique() != null ? getStringValue(profile.getPhysique().getHeight()) : ""),
					profile.getMaritalStatus(), qual, workPlace, det1, det2, info };
		} else {
			strMat = new String[] { getStringValue(id), "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
					"", "", "", "", info };
		}
